...
import 'react-toastify/dist/ReactToastify.css';
 ...
 export const Room3Screen = () => {
  ...
  const handleReservation3Success = () => {
    if(error.length > 1){
 ...
 useEffect(() => {
   db.collection("reservations").where("room", "==", 3).onSnapshot((snapshot) => {
    let temp =[];
  ...
  const reserveHandler = async () => {
    ...
    await db.collection("reservations").add({date: moment(value).format("DD-MM-YYYY"), userId: currentUser.uid, room: 3})
  }
